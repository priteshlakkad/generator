...
public class RemoteResourceLoader implements FSStreamFactory {
	...
	private byte[] fetch(String uri) {
		if (!isHostAllowed(uri)) {
			log.warn("Blocked resource from a host that is not in quoteweave.render.allowed-hosts: {}", uri);
			return TRANSPARENT_PIXEL;
		}
		try {
			...
			connection.setRequestProperty("User-Agent", "quoteweave-platform");

			try (InputStream in = connection.getInputStream()) {
				byte[] body = readAtMost(in, properties.getMaxResourceBytes());
				if (body == null) {
					log.warn("Resource exceeds quoteweave.render.max-resource-bytes ({} bytes), skipped: {}",
